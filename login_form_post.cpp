...
int main()
{
    ...
    // Récupération de la longueur des données transmises via POST
    std::string postData;
    ...
    // Extraction les valeurs des champs login et password du POST
    std::string extractedLogin, extractedPassword;
    size_t loginPos = postData.find("login=");
    size_t passwordPos = postData.find("&password=");

    if (loginPos != std::string::npos && passwordPos != std::string::npos)
    {
        extractedLogin = urlDecode(postData.substr(loginPos + 6, passwordPos - (loginPos + 6)));
        extractedPassword = urlDecode(postData.substr(passwordPos + 10));
    }
    ...
    else
    {
        ...
        std::cout << "<form method='post' action='/cgi-bin/login_form.cgi'>";
